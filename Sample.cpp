...
int main()
{
    ...
    reader.getPathsFromBuffer(json, jsonPaths);
    ...
    // Get user names.
    reader.onPair("name", [&user](const char* name)
    {
        user.name = name;
    });
    // Get user identifiers.
    reader.onPair("id", [&user](const char* id)
    {
        user.id = std::atoi(id);
    });
    // Called once a new user has been parsed.
    reader.onArrayItem("users", [&users, &user](const char*)
    {
        users.push_back(user); // Store user info.
    });
